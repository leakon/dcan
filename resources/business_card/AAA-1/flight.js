	...
	$.extend($.XYTipsWindow,{
		...
		init: function (o){
			...
			if(o.___showbg != "" && o.___showbg == true){
				var $boxBgDom = "<div id=\"XYTipsWindowBg"+o.___boxID+"\" style=\"position:absolute;background:"+o.___windowBgColor+";filter:alpha(opacity=0);opacity:0;width:100%;left:0;top:0;z-index:870611\"><iframe src=\"about:blank\" style=\"width=100%;height:"+$(document).height()+"px;filter:alpha(opacity=0);opacity:0;scrolling=no;z-index:870610\"></iframe></div>";
				$($boxBgDom).appendTo("body").animate({opacity:o.___windowBgOpacity},200).end();
				$("#XYTipsWindowBg"+o.___boxID).dblclick(function(){
					$.XYTipsWindow.removeBox();
				});
			};
			if(o.___drag != "") {
		...
		removeBox: function (){
			...
			var $boxbg = $("#XYTipsWindowBg"+BOXID.___boxID);
	...
	for(i=0;i<n;i++){
		...
		if ($(objt).width() > Img_Maxwidth){
			...
			$('.showimgbtn').click(function() {
				...
				$(".theImg").remove();
				$.XYTipsWindow({
					___title: "图片:"+Img_name,
					___content:	"img:"+Img_src,
					___width: Img_width,
					___height: Img_height,
					___drag: "___boxTitle",
					___showbg: false
				});
				return false;
